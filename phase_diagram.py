...
def dimer_order_parameter(L, D, Jz,J, delta_z_values, delta, h, datas=True):
    """
    Compute the dimer order parameter as a function of delta_z
    Parameters:
    ----------
    L : number of sites
    D : bond dimension
    ...
    for delta_z in delta_z_values:
        ...
        dimer_z_order.append(np.abs(mps.Dimer_Z_order_parameter(L//2)))
        dimer_xy_order.append(np.abs(mps.Dimer_XY_order_parameter(L//2)))
    
    # Plot the dimer order parameters
    plt.figure(figsize=(8, 6))
    plt.plot(delta_z_values, dimer_z_order, marker='s', label=r'$D_z(\frac{L}{2})$')
    plt.plot(delta_z_values, dimer_xy_order, marker='^', label=r'$D_{XY}(\frac{L}{2})$')
    plt.xlabel(r'$\delta_z$')
    plt.ylabel('Dimer Order Parameter at L/2')
    plt.title(r'Dimer Order Parameter vs $\delta_z$')
    plt.legend()
    plt.grid()
    plt.show()
...
def correlation_length_vs_deltaz(L, D, Jz, J, delta_z_values, delta, h, datas=True, plot=True):  
    ...
    if plot:
        ...
        plt.figure(figsize=(8, 6))
        plt.plot(delta_z_values, correlation_lengths, marker='s')
        plt.xlabel(r'$\delta_z$')
        plt.ylabel(r" $\xi^{-1}$")
        plt.title(r'Inverse correlation Length vs $\delta_z$')
        plt.grid()
        plt.show()
...
def critical_phase_Diagram(L, D, Jz, J, delta_z_values, delta_values, h, datas=True):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(critical_points, delta_values, marker='s', label='Critical Points')
    x_fit = np.linspace(min(critical_points), max(critical_points), 200)
    plt.plot(x_fit, linear_func(x_fit, a, b), 'r--', label=f'Fit: delta = {a:.4f} * delta_z + {b:.3f}')
    plt.xlabel(r'$\delta_z$')
    plt.ylabel(r'$\delta$')
